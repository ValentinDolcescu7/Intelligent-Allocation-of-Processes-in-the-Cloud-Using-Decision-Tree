...
def find_valid_combinations():
    valid_combinations = []
    for bandwidth in range(1, 25):
        for delay in range(71, 101):
...
def add_additional_column(combinations):
    ...
    for index in range(len(combinations)):
        if index % 3 == 0 and index != 0:
            additional_value -= 0.12
        combinations[index] = combinations[index] + (additional_value,)
    
    combinations[-1] = combinations[-1][:3] + (1.0,)
...
save_to_csv(valid_combinations_sorted, r'D:\Licenta\Licenta\Cloud\generate_dataset\low_score.csv')
